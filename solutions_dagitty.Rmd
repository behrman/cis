...
```{r echo=FALSE}
v <- sessioninfo::package_info("dagitty", dependencies = FALSE)
```

The code below used version `r v$loadedversion` of dagitty. If the CRAN version is less than this version, the current development version can be installed with:
